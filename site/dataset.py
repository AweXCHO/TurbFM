...
class ToTensor(object):
    def __call__(self, sample, mu=0.5, sigma=0.5):
        ...
        label_cn2 = torch.from_numpy(label_cn2).float() / (2.177*1e-13) # physics
        ...
        label_integral = torch.from_numpy(label_integral).float() / (4.158*1e-12) # pysics
        # label_integral = torch.from_numpy(label_integral).float() * 2.5e11 # Antarctica

        sample['input'] = torch.from_numpy(input).float() / 255.0
        sample['gt'] = torch.from_numpy(gt).float() / 255.0
