...
class SwinMAE(nn.Module):
    ...
    def __init__(self, img_size: int = 224, patch_size: int = 4, mask_ratio: float = 0.75, in_chans: int = 3,
        ...
        self.pretrain_pth = '/ayt1/anyitong/TurbMAE/2D/PBCL+83_MAE/data/checkpoints/83-checkpoint-499.pth'
        self.init_weights()
    ...
    # 改进1：多尺度 Patch Embedding 模块 + 扰动感知 Mask 策略
    def forward_encoder(self, x):
        x = self.patch_embed(x)
        
        x, mask = self.grad_based_masking(x, remove=False, mask_len_sparse=False)
    ...
    def compute_patch_gradients(self, patches, patch_size=16, eps=1e-6):
        B, L, D = patches.shape
        P = int((D // 3) ** 0.5)  # 假设每 patch 是 P×P×3
        img = patches.view(B, L, 3, P, P)
        gray = img.mean(dim=2)  # [B, L, P, P]

        dx = gray[:, :, :, 1:] - gray[:, :, :, :-1]
        dy = gray[:, :, 1:, :] - gray[:, :, :-1, :]

        dx_mean = dx.mean(dim=[2, 3])
        dy_mean = dy.mean(dim=[2, 3])
        return dx_mean, dy_mean

    def tga_loss(self, pred, target, mask, patch_size=16, eps=1e-6):
        dx_p, dy_p = self.compute_patch_gradients(pred, patch_size)
        dx_t, dy_t = self.compute_patch_gradients(target, patch_size)

        norm_p = torch.sqrt(dx_p ** 2 + dy_p ** 2 + eps)
        norm_t = torch.sqrt(dx_t ** 2 + dy_t ** 2 + eps)

        mag_pred = norm_p
        mag_target = norm_t
        mag_loss = torch.abs(mag_pred - mag_target)

        loss_mag = (mag_loss * mask).sum() / (mask.sum())
        return loss_mag
    ...
    #     mse_loss_masked = (mse_loss * mask).sum() / (mask.sum())

    #     # ====== TGA Loss（方向 + 幅值）=====
    #     mag_loss = self.tga_loss(pred, target, mask)

    #     # print(mse_loss_masked, mag_loss)
    #     # 重新加权
    #     total_loss = mse_loss_masked + min(0.05, 3 * mag_loss)
