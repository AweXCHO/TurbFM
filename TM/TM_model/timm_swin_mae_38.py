...
class MultiScalePatchEmbedding(nn.Module):
    def __init__(self, patch_sizes=[4, 8], in_c=3, embed_dim=96, norm_layer=None, use_freq_token=True):
        super().__init__()
        self.patch_sizes = patch_sizes
        self.embed_dim = embed_dim
        self.use_freq_token = use_freq_token
        self.total_branches = len(patch_sizes)
        self.branch_dim = embed_dim // len(patch_sizes)
        self.branches = nn.ModuleList()

        for p in patch_sizes:
            self.branches.append(
                nn.Sequential(
                    nn.Conv2d(in_c, self.branch_dim, kernel_size=p, stride=p),
                )
            )

        # 频域投影通道匹配 branch_dim
        self.freq_proj = nn.Conv2d(in_c, self.branch_dim, kernel_size=1) if use_freq_token else None

        # concat 后维度：embed_dim + branch_dim (if freq_token used)
        concat_dim = embed_dim + (self.branch_dim if use_freq_token else 0)
        self.norm = norm_layer(concat_dim) if norm_layer else nn.Identity()

        # 融合映射回 embed_dim（关键步骤）
        self.fuse_proj = nn.Linear(concat_dim, embed_dim)

    def extract_freq_token(self, x, target_H, target_W):
        """
        使用 torch.fft.fft2 提取频域幅值，并调整为目标 spatial 尺寸。
        输入 x: [B, C, H, W]
        输出 freq_token: [B, target_H, target_W, branch_dim]
        """
        B, C, H, W = x.shape

        freq = torch.fft.fft2(x, norm='ortho')       # [B, C, H, W], complex
        freq_mag = torch.abs(freq)                   # [B, C, H, W]
        ...
        # 通道映射
        freq_token = self.freq_proj(resized)         # [B, branch_dim, H', W']
        freq_token = rearrange(freq_token, 'B C H W -> B H W C')
        return freq_token
    ...
    def forward(self, x):
        ...
        for branch in self.branches:
            feat = branch(x)                          # [B, branch_dim, H', W']
            feat = rearrange(feat, 'B C H W -> B H W C')
            outs.append(feat)

        # pad 各尺度以对齐空间尺寸
        max_H = max(f.shape[1] for f in outs)
        max_W = max(f.shape[2] for f in outs)

        for i in range(len(outs)):
            h_pad = max_H - outs[i].shape[1]
            w_pad = max_W - outs[i].shape[2]
            if h_pad > 0 or w_pad > 0:
                outs[i] = F.pad(outs[i], (0, 0, 0, w_pad, 0, h_pad))  # pad last 2 dims

        patch_tokens = torch.cat(outs, dim=-1)  # [B, H, W, embed_dim]

        if self.use_freq_token:
            freq_token = self.extract_freq_token(x, max_H, max_W)     # [B, H, W, branch_dim]
            # print("patch_tokens dtype:", patch_tokens.dtype)
            # print("freq_token dtype:", freq_token.dtype)
            tokens = torch.cat([patch_tokens, freq_token], dim=-1)    # [B, H, W, embed_dim + branch_dim]
        else:
            tokens = patch_tokens                                     # [B, H, W, embed_dim]

        tokens = self.norm(tokens)
        fused = self.fuse_proj(tokens)                                # [B, H, W, embed_dim]
        return fused
...
class SwinMAE(nn.Module):
    ...
    def init_weights(self):
        checkpoint_path = '/home/ayt/TurbMAE/TM/DATUM+MAE/MAE_checkpoints/38-checkpoint-499.pth'
        checkpoint = torch.load(checkpoint_path, map_location='cpu')
