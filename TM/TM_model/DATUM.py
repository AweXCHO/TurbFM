...
# from .timm_swin_mae_81 import SwinMAE
# from .timm_swin_mae_38 import SwinMAE
# from .timm_swin_mae_83 import SwinMAE
# from .timm_swin_mae_92 import SwinMAE
...
class UpsampleBlock(nn.Module):
    def __init__(self, in_channels, out_channels, scale_factor, activation):
        super().__init__()
...
class FeatureRefiner(nn.Module):
    def __init__(self, para):
        ...
        self.refine1 = UpsampleBlock(256, 1 * self.n_feats, scale_factor=8, activation=self.activation)
        self.refine2 = UpsampleBlock(512, 2 * self.n_feats, scale_factor=8, activation=self.activation)
        self.refine3 = UpsampleBlock(1024, 5 * self.n_feats, scale_factor=8, activation=self.activation)

    def forward(self, feat1, feat2, feat3):
        # featX: [B, T, H, W, C]
        B, T, H1, W1, C1 = feat1.shape
        _, _, H2, W2, C2 = feat2.shape
        _, _, H3, W3, C3 = feat3.shape

        # Flatten B and T
        x1 = feat1.reshape(B * T, H1, W1, C1).permute(0, 3, 1, 2)  # [B*T, C, H, W]
        x2 = feat2.reshape(B * T, H2, W2, C2).permute(0, 3, 1, 2)
        x3 = feat3.reshape(B * T, H3, W3, C3).permute(0, 3, 1, 2)

        # Apply upsampling
        out1 = self.refine1(x1).reshape(B, T, -1, H1 * 8, W1 * 8)
        out2 = self.refine2(x2).reshape(B, T, -1, H2 * 8, W2 * 8)
        out3 = self.refine3(x3).reshape(B, T, -1, H3 * 8, W3 * 8)

        return out1, out2, out3
    

class FeatureRefiner_orimae(nn.Module):
    def __init__(self, para):
        super().__init__()

        self.n_feats = para.n_features        # = 16
        self.activation = para.activation

        in_dim = 768

        # 1️⃣ 先把 token 转成 feature map
        self.to_map = nn.Conv2d(in_dim, in_dim, kernel_size=1)

        # 2️⃣ 三个尺度（卷积生成不同通道金字塔）
        self.conv1 = nn.Conv2d(in_dim, 1 * self.n_feats, kernel_size=3, padding=1)
        self.conv2 = nn.Conv2d(in_dim, 2 * self.n_feats, kernel_size=3, padding=1)
        self.conv3 = nn.Conv2d(in_dim, 5 * self.n_feats, kernel_size=3, padding=1)

        # 3️⃣ 上采样结构（完全参考你提供的设计）
        self.upsample1 = nn.Upsample(scale_factor=16, mode='bilinear', align_corners=False)
        ...
        self.act1 = actFunc(self.activation)
        self.act2 = actFunc(self.activation)
        self.act3 = actFunc(self.activation)

    def forward(self, x):
        """
        x: [T, N, C]  e.g. [15, 196, 768]
        return: 3 tensors: [1, T, Ck, Hk, Wk]
        """
        ...
        # ------ 恢复 2D 网格 ------
        H = W = int(math.sqrt(N))
        assert H * W == N, "Token count must be square."
        ...
        # 三级特征
        f1 = self.act1(self.conv1(x))     # [B*T, 16, H, W]
        f2 = self.act2(self.conv2(x))     # [B*T, 32, H, W]
        f3 = self.act3(self.conv3(x))     # [B*T, 80, H, W]

        # 上采样（不写死，只保持比例）
        o1 = self.upsample1(f1)
        o2 = self.upsample2(f2)
        o3 = self.upsample3(f3)

        # reshape → [B, T, C, H, W]
        o1 = rearrange(o1, '(b t) c h w -> b t c h w', b=1, t=T)
        o2 = rearrange(o2, '(b t) c h w -> b t c h w', b=1, t=T)
        o3 = rearrange(o3, '(b t) c h w -> b t c h w', b=1, t=T)
...
def match_feature_statistics(f_mae, f_orig, eps=1e-5):
    mean_orig = f_orig.mean(dim=(0, 1, 2, 3, 4), keepdim=True)
    std_orig = f_orig.std(dim=(0, 1, 2, 3, 4), keepdim=True)

    mean_mae = f_mae.mean(dim=(0, 1, 2, 3, 4), keepdim=True)
    std_mae = f_mae.std(dim=(0, 1, 2, 3, 4), keepdim=True)

    f_mae_normed = (f_mae - mean_mae) / (std_mae + eps)
    f_mae_scaled = f_mae_normed * std_orig + mean_orig

    # 标准化后的统计
    mean_aligned = f_mae_scaled.mean(dim=(0, 1, 2, 3, 4), keepdim=True)
    std_aligned = f_mae_scaled.std(dim=(0, 1, 2, 3, 4), keepdim=True)

    # 打印信息
    # print(f'[{name}] Feature Statistics Comparison:')
    # print(f'  Original Encoder     - Mean: {mean_orig.item():.4f}, Std: {std_orig.item():.4f}')
    # print(f'  MAE Encoder (raw)    - Mean: {mean_mae.item():.4f}, Std: {std_mae.item():.4f}')
    # print(f'  MAE Aligned (scaled) - Mean: {mean_aligned.item():.4f}, Std: {std_aligned.item():.4f}')
    # print('-' * 60)
    return f_mae_scaled
...
class FeatureAlign(nn.Module):
    ...
    def _initialize(self, x):
        ...
        device = x.device
        # 创建 LayerNorm
        self.ln = nn.LayerNorm(dim).to(device)
        # 创建 learnable γ / β
        self.gamma = nn.Parameter(torch.ones(1, 1, 1, 1, dim, device=device))
        self.beta  = nn.Parameter(torch.zeros(1, 1, 1, 1, dim, device=device))
...
class SimpleFusion(nn.Module):
    ...
    def forward(self, f1, f2):
        """
        f1, f2: 形状相同的张量 (B, T, C, H, W)
        return: 融合后的张量，同形状 (B, T, C, H, W)
        """
        # 检查尺寸一致
        assert f1.shape == f2.shape, "Features must have the same shape!"
        # 可学习加权融合
        # print(f1,f2)
        fused = self.alpha * f1 + (1 - self.alpha) * f2
        return fused
...
class ConvFusion(nn.Module):
    def __init__(self):
        super().__init__()
        # 输入通道=2 (两个特征)，输出通道=1
        self.conv = nn.Conv3d(in_channels=2, out_channels=1, kernel_size=1)

    def forward(self, f1, f2):
        # print("weight:", self.conv.weight.data.view(-1))
        # print("bias:", self.conv.bias.data)
        # print(f1,f2)
        # f1,f2 shape: (B, T, C, H, W)
        B, T, C, H, W = f1.shape
        # stack -> (B, 2, T, C, H, W)
        x = torch.stack([f1, f2], dim=1)
        # merge T*C -> new depth dimension
        x = x.reshape(B, 2, T*C, H, W)  # now (B, 2, D, H, W)
        # conv3d
        fused = self.conv(x)            # (B, 1, T*C, H, W)
        # reshape back
...
class AttentionFusion(nn.Module):
    def __init__(self, reduction=4):
        ...
        # 门控机制：全局统计 -> 通道注意力
        self.gate_fc1 = nn.Linear(2, 2 // 1)
        self.gate_fc2 = nn.Linear(2 // 1, 2)
        self.sigmoid = nn.Sigmoid()

    def forward(self, f1, f2):
        """
        f1, f2: 输入特征, 形状必须相同 (B, T, C, H, W)
        """
        assert f1.shape == f2.shape
        # Global average pooling across space: -> (B,T,C)
        g1 = f1.mean(dim=(-1, -2))  # 取 spatial mean
        g2 = f2.mean(dim=(-1, -2))
        g = torch.stack([g1, g2], dim=-1)  # (B, T, C, 2)
        # Compute gating weights
        g = self.gate_fc1(g)     # (B, T, C, hidden)
        g = torch.relu(g)
        g = self.gate_fc2(g)     # (B, T, C, 2)
        weights = self.sigmoid(g)
        # weight split
...
class Model(nn.Module):
    def __init__(self, para):
        ...
        # self.spynet = SpyNet()
        # self.mae = SwinMAE()
        # self.converter = FeatureRefiner(para)
        self.mae = mae_vit_base_patch16_dec512d8b()
        self.converter = FeatureRefiner_orimae(para)
        ...
        # self.align1 = FeatureAlign()
        # self.align2 = FeatureAlign()
        # self.align3 = FeatureAlign()
        self.fusion1 = AttentionFusion()
        self.fusion2 = AttentionFusion()
        self.fusion3 = AttentionFusion()
    ...
    def forward(self, x):
        ...
        # x = x.flatten(0,1)
        # f1, f2, f3 = self.mae(x)
        # f1, f2, f3 = f1.unflatten(0, (b, l)), f2.unflatten(0, (b, l)), f3.unflatten(0, (b, l))
        # # print(f1.shape, f2.shape, f3.shape)
        # f1, f2, f3 = self.converter(f1, f2, f3)

        # #  ourMAE特征标准化后与原特征加权融合
        # f1_orig, f2_orig, f3_orig = self.get_features(x)
        # # print(f1_orig.shape, f2_orig.shape, f3_orig.shape)
        # b, l, _, _, _ = x.shape
        # x_flat = x.flatten(0, 1)  # 变成 (b*l, 3, 224, 224)
        # f1_mae, f2_mae, f3_mae = self.mae(x_flat)
        # f1_mae = f1_mae.unflatten(0, (b, l))
        # f2_mae = f2_mae.unflatten(0, (b, l))
        # f3_mae = f3_mae.unflatten(0, (b, l))
        # f1_mae, f2_mae, f3_mae = self.converter(f1_mae, f2_mae, f3_mae)

        # oriMAE特征标准化后与原特征加权融合
        f1_orig, f2_orig, f3_orig = self.get_features(x)
        # print(f1_orig.shape, f2_orig.shape, f3_orig.shape)
        b, l, _, _, _ = x.shape
        x_flat = x.flatten(0, 1)  # 变成 (b*l, 3, 224, 224)
        f_mae = self.mae(x_flat)
        # f1_mae = f1_mae.unflatten(0, (b, l))
        # f2_mae = f2_mae.unflatten(0, (b, l))
        # f3_mae = f3_mae.unflatten(0, (b, l))
        f1_mae, f2_mae, f3_mae = self.converter(f_mae)
        # print(f1_mae.shape, f2_mae.shape, f3_mae.shape)


        f1 = self.fusion1(f1_orig, f1_mae)
        f2 = self.fusion2(f2_orig, f2_mae)
        f3 = self.fusion3(f3_orig, f3_mae)


        # # ==== 特征对齐方式 ====
        # f1_mae_align = self.align1(f1_mae)
        # f2_mae_align = self.align2(f2_mae)
        # f3_mae_align = self.align3(f3_mae)
        # # ==== 融合 ====
        # alpha = 0.5
        # f1 = alpha * f1_orig + (1 - alpha) * f1_mae_align
        # f2 = alpha * f2_orig + (1 - alpha) * f2_mae_align
        # f3 = alpha * f3_orig + (1 - alpha) * f3_mae_align
        ...
        return prepare_reverse(False, True, out)
...
def get_args():
    ...
    parser.add_argument('--spynet_path', type=str, default="spynet_init.pth")
...
if __name__ == "__main__":
    ...
    data = torch.randn((2,15,3,224,224)).to(device=device)
