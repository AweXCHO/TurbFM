...
class ToTensor(object):
    def __call__(self, sample, mu=0.5, sigma=0.5):
        ...
        label_cn2 = np.ascontiguousarray(label_cn2).transpose((2,0,1,3))
        label_cn2 = torch.from_numpy(label_cn2).float() / 1.63e-9 # physics
        # label_cn2 = torch.from_numpy(label_cn2).float() * 5e12 # Antarctica
        label_integral = np.ascontiguousarray(label_integral).transpose((2,0,1,3))
        label_integral = torch.from_numpy(label_integral).float() / 3.02e-8 # pysics
...
class VideoFrameDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label_cn2 = scio.loadmat(os.path.join(labels_path, lables_list[5]))
        label_cn2 = label_cn2['Cn2_Mat_sequence']
        # (14, 14, 30, 15) -> 提取中心temporal_len帧
        total_frames = label_cn2.shape[-1]
        start_frame = (total_frames - self.temporal_len) // 2
        label_cn2 = label_cn2[:, :, :, start_frame:start_frame + self.temporal_len]
        # (14, 14, 30, temporal_len)

        label_integral = scio.loadmat(os.path.join(labels_path, lables_list[6]))
        label_integral = label_integral['integral3dMat']
        # (14, 14, 5, 15) -> 提取中心temporal_len帧
        total_frames = label_integral.shape[-1]
        start_frame = (total_frames - self.temporal_len) // 2
        label_integral = label_integral[:, :, :, start_frame:start_frame + self.temporal_len]
        # (14, 14, 5, temporal_len)
        ...
        for vid_index in range(5):
            ...
            for frame_index in range(self.temporal_len):
                video.set(cv2.CAP_PROP_POS_FRAMES, start_frame + frame_index)
                vid[vid_index, frame_index, :, :, :] = video.read()[1]
            video.release()
